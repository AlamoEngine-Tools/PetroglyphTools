...
internal class MtdFileBuilder : IBinaryFileBuilder<MtdFile, MtdFileHolder>
{
    ...
    public MtdFile FromBytes(byte[] byteStream)
    {
        ...
        var header = BuildMtdHeaderInternal(byteStream);
        var imageTable = BuildMtdImageTableInternal(byteStream);
    ...
    private static MtdImageTable BuildMtdImageTableInternal(byte[] byteStream)
    {
        ...
        for (var i = IMAGE_TABLE_START_OFFSET; i < byteStream.Length; i += RECORD_SIZE)
        {
            var currentOffset = i;
            var name = Encoding.ASCII.GetString(byteStream, currentOffset, MtdImageTableRecord.BINARY_NAME_LENGTH);
